#include <stdio.h>
/*
This is an adapted version of uxnasm created by Devine Lu Linvega.
The original source code can be found here: https://git.sr.ht/~rabbits/uxn/tree/main/item/src/uxnasm.c
uxnasm is licensed under the MIT License
*/
#define LENGTH 0x10000

unsigned int TRIM = 0x0400;
...
static int
writeshort(Uint16 s, int ldi)
{
	if(ldi)
		if(!writebyte(findopcode("ldi2"))) return 0;
	return writebyte(s & 0xFF) && writebyte(s >> 8);
}

static int
writeldibyte(Uint8 b)
{
	if(!writebyte(findopcode("ldi"))) return 0;
...
parse(char *w, FILE *f)
{
	...
	case '#': /* literals hex */
		...
		if(slen(w) == 3) {
			if(!writeldibyte(shex(w + 1))) return 0;
		} else if(slen(w) == 5) {
    ...
    case '`': /* origin */
        if(!sihx(w + 1))
            return error("Invalid origin", w);
        TRIM = shex(w + 1);
        break;
    case '.': /* literal byte zero-page */
		makereference(p.scope, w, p.ptr);
		if(!writeldibyte(0xff)) return 0;
		break;
...
review(char *filename)
{
	...
	fprintf(stderr,
		"Assembled %s in %d bytes, %d labels, %d macros.\n",
		filename,
		p.length - TRIM,
		p.llen,
		p.mlen);
...
main(int argc, char *argv[])
{
	FILE *src, *dst;
	if(argc < 3)
		return !error("usage", "input.S output.bin");
	if(!(src = fopen(argv[1], "r")))
		return !error("Invalid input", argv[1]);
	if(!assemble(src))
		return !error("Assembly", "Failed to assemble.");
	if(!(dst = fopen(argv[2], "wb")))
		return !error("Invalid Output", argv[2]);
	if(p.length <= TRIM)
		return !error("Assembly", "Output binary is empty.");
